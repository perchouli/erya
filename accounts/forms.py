...
class RegistrationForm(forms.Form):
    email = forms.EmailField(error_messages = {'required': u'E-mail不能为空'})
    username = forms.CharField(error_messages = {'required': u'用户名不能为空', 'max_length':u'最多是8个字'}, max_length = 16)
    ...
    def clean(self):
        if ('confirm_password' in self.cleaned_data) and ('password' in self.cleaned_data):
            if (self.cleaned_data['confirm_password'] != self.cleaned_data['password']):
                self._errors["confirm_password"] = ErrorList([u'密码和确认密码不一致'])
